...
//Company Controler
module.controller('CompanyCtrl', ['$scope', '$rootScope', '$routeParams', 'Model', 'Auth', '$timeout', function ($scope, $rootScope, $routeParams, Model, Auth, $timeout) {
	...
	$scope.companyInfoEdit = function (conmpanyInfo) {
		...
		$scope.editInfoError = '';
 		Model.companyInfoEdit(conmpanyInfo).then(function (data) {
 			...
 			$timeout(function () {
				$scope.editInfoError = true;
			})
		}).finally(function () {
		});
	};
	
	$scope.registerLetterInit = function () {
	...
	$scope.setCity= function (addNewRequest) {
 		if(addNewRequest.State){
			Model.cityList(addNewRequest.State).then(function (data) {
 				addNewRequest.AllCity=data.data;
			}, function (error) {
			}).finally(function () {
			});
		}
	...
	$scope.checkAndRegister = function () {
		$scope.allErrorRequest=[];
		
		var validataion=false;
		$.each($scope.allRequest,function(index,value){
 			if(!value.success){
				//some validation
		...
		if(validataion && $scope.allRequest && $scope.allRequest[0]){
			$scope.addNewRequestFunction(0);
		} 
	...
	$scope.addNewRequestFunction = function (requestId) {

		if(requestId >= $scope.allRequest.length){
			$scope.percent = 100;
		...
		if(requestId==0){
			...
			$scope.percent = ((requestId*100)/$scope.allRequest.length).toFixed(0);
		}



		var addNewRequest= $scope.allRequest[requestId];
		if (request == true) return;

		request = true;


		if (addNewRequest.Floor && addNewRequest.Floor[0]) {
			addNewRequest.Floor = addNewRequest.Floor[0]
		} else {
			addNewRequest.Floor = '';
		}

		//send new requset to server
		Model.requestNew(addNewRequest).then(function (data) {

			addNewRequest.success="عملیات با موفقت انجام شد";
			addNewRequest.error =undefined; 

			$scope.allSuccessRequest.push($scope.allRequest[requestId]);
			$scope.allRequest.splice(requestId,1)
			console.log($scope.allSuccessRequest)
			//server response is url of bank , we redirect user to this url
			//window.location.href = data.redirect_url;
		}, function (error) {
			$scope.allErrorRequest.push($scope.allRequest[requestId]);

			if (error.error = 'NOT_FOUND') {
				addNewRequest.error =  "اطلاعات وارد شده صحیح نمی باشد ، لطفا مجددا اطلاعات را با دقت وارد نمایید"
			}
			else {
				addNewRequest.error =   "با عرض پوزش ، خطایی رخ داده است ، لطفا کمی بعد مجددا اقدام نمایید."
			}
			addNewRequest.success =undefined;

		}).finally(function () {
			request = false;
			$scope.addNewRequestFunction(++requestId)
		});
	};

	//loading (...)
	function startLoadingLoop(){
		$scope.loading=setInterval(function(){
			if($scope.loadingPoint==''||$scope.loadingPoint==undefined){
				$scope.loadingPoint='.';
			}
			else if($scope.loadingPoint=='.'){
				$scope.loadingPoint='..';
			}
			else if($scope.loadingPoint=='..'){
				$scope.loadingPoint='...';
	...
	$('input').on('input', function () {
 	...
 	$("#uploadCSV").on("change", function (oEvent) {
	...
	function fitExcel(excel) {
		...
		$.each(excel,function (index,value) {
 			var addNewRequest = {
				...
				City: value.City,
				//VillageDrop: value.VillageDrop,
				//VillageTxt: value.VillageTxt,
				//MainRoadDrop: value.MainRoadDrop,
				//MainRoadTxt: value.MainRoadTxt,
				//SecondaryRoadDrop1: value.SecondaryRoadDrop1,
				//SecondaryRoadTxt1: value.SecondaryRoadTxt1,
				//SecondaryRoadDrop2: value.SecondaryRoadDrop2,
				//SecondaryRoadTxt2: value.SecondaryRoadTxt2,
				//Plaque: value.Plaque,
				//Floor: value.Floor,
				//Unit: value.Unit,
				//BuildingName: value.BuildingName
			};
			$scope.allRequest.push(addNewRequest);
		})

		console.log($scope.allRequest)
		$scope.registerLetterInit();
		$scope.$apply();
	}
